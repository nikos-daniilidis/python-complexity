...
from VarianceEstimator import VarianceEstimator as VE
...
from config import GREEDY, EPS_GREEDY, SOFTMAX, THOMPSON, THOMPSON_SOFTMAX, BOOTSTRAP

__author__ = "nikos.daniilidis"


def main_loop(note, readme, event_types, seed_events, update_events, analysis_events, ps, noise_spread, seeds,
              gs, num_inputs, classifier_kinds, ve_config, train_args, criterion, method, tau_dict, batch_updates):
    """
    ...
    :param ve_config: dict. Configuration for variance estimator to use for the classifiers
    ...
    :param method: string. Method used to select between multiple streams (greedy, thompson, ...)
    :param tau_dict: dict. Mapping from batch update indices to softmax tau (temperature) values
    :param batch_updates: int. Number of batch updates to run for the models
    :return: Nothing
    """
    for b in range(batch_updates):
        assert b in tau_dict.keys()
        assert isinstance(tau_dict[b], float)
    ...
    if not os.path.exists(dirname):
        os.makedirs(dirname)
    save_metadata(event_types=event_types, seed_events=seed_events, update_events=update_events,
                  analysis_events=analysis_events, ps=ps, noise_spread=noise_spread, seeds=seeds, num_inputs=num_inputs,
                  classifier_kind=classifier_kinds, ve_config=ve_config, criterion=criterion, method=method,
                  batch_updates=batch_updates, file_descriptor=file_descriptor, dirname=dirname, note=note,
                  tau_dict=tau_dict, readme=readme)
    pn = plot_namer(dirname=dirname)
    pn_csv = plot_namer(dirname=dirname, suffix='.csv')

    # EventGenerators
    mother_eg = EG(seed=42, num_inputs=num_inputs, spread=noise_spread)
    egs = []
    ...
    # VarianceEstimator
    ve = VE(kind=ve_config["kind"])

    # EventSelector
    es = ES(selection_type=criterion, method=method)
    ...
    for batch_update in range(batch_updates):
        ...
        if batch_update == 0:
            xs, ys = es.filter(xs=xrs, ys=yrs,
                               models=nones, sd_estimator=None, event_gains=gs)
            ...
            xs, ys = es.filter(xs=xrs, ys=yrs,
                               models=ms, sd_estimator=ve, event_gains=gs)
            xafs, yafs = es.filter(xs=xas, ys=yas,
                                   models=ms, sd_estimator=ve, event_gains=gs)
            xr, yr = es.filter(xs=xrs, ys=yrs,
                               models=nones, sd_estimator=None, event_gains=gs)  # stream of randomly chosen events,
            # no models or sd_estimator needed
        ...
        for ix, mu in enumerate(mus):
            ...
            if ms[ix] is not None:
                b_p.append(ms[ix].best_params_)
            else:
                b_p.append(None)
            print ms[ix].best_params_

        # update model variance estimator using new models and new data
        if method in (THOMPSON, THOMPSON_SOFTMAX):
            if ve_config["kind"] == BOOTSTRAP:
                replace = ve_config["replace"]
            else:
                replace = None
            ve.train(xs=Xus, ys=Yus, ms=ms, replace=replace, predictor=None)
        ...
        # sdas = ve.predict_sd(xas)  # standard deviation estimates for each model on each data point
        if method in (GREEDY, EPS_GREEDY, THOMPSON):
            xafnews, yafnews = es.filter(xs=xas, ys=yas, models=ms, sd_estimator=ve, event_gains=gs)
        elif method in (SOFTMAX, THOMPSON_SOFTMAX):
            tau = tau_dict[batch_update]
            xafnews, yafnews = es.filter(xs=xas, ys=yas, models=ms, sd_estimator=ve, event_gains=gs, tau=tau)
