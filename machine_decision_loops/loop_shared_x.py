...
def main():
    ...
    seed_events = 6000  # number of events to use on the first round of training
    ...
    for batch_update in range(batch_updates):
        ...
        for ix, mu in enumerate(mus):
            ms.append(mu.train(Xus[ix], Yus[ix],
                               learning_rate=[0.01, 0.03, 0.06, 0.08, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35],
                               n_estimators=[600],
                               subsample=0.5, max_depth=[2, 3, 4, 5], random_state=13, scoring='neg_log_loss'))
